...
DB_MODE = os.environ.get("DB_MODE", "cloud")  # "cloud" or "local"

if DB_MODE == "local":
    # Local Postgres, no Cloud SQL Connector needed at all
    from sqlalchemy import create_engine
    LOCAL_DB_USER = os.environ.get("LOCAL_DB_USER", "edikte_app")
    LOCAL_DB_PASSWORD = os.environ["LOCAL_DB_PASSWORD"]
    LOCAL_DB_NAME = os.environ.get("LOCAL_DB_NAME", "edikte_analytics")
    engine = create_engine(
        f"postgresql+pg8000://{LOCAL_DB_USER}:{LOCAL_DB_PASSWORD}@localhost:5432/{LOCAL_DB_NAME}"
    )
